...
export class UserDAO extends BaseDAO {
  async findAll(limit: number = 10, offset: number = 0): Promise<User[]> {
    const sql = 'SELECT id, username, email, created_at FROM users ORDER BY id LIMIT $1 OFFSET $2';
    const params = [limit, offset];
    const rows = await this.executeQuery<{ id: number; username: string; email: string; created_at: string }[]>(sql, params);
  ...
  async create(username: string, email: string): Promise<User> {
    const sql = 'INSERT INTO users (username, email) VALUES ($1, $2) RETURNING id, username, email, created_at';
  ...
  async update(id: number, username: string, email: string): Promise<User | null> {
    const sql = 'UPDATE users SET username = $1, email = $2 WHERE id = $3 RETURNING id, username, email, created_at';
